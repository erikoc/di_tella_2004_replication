...
def test_regression_WeeklyPanel(input_data_regression_WeeklyPanel):
    params = regression_WeeklyPanel(
        input_data_regression_WeeklyPanel,
        "y_variable",
        "clustered",
    )
...
def test_testings_areg_single(
    input_data_testings,
):  # We use the testing applied to the areg_single regression below tested
    result = areg_single(
        Data=input_data_testings,
        variable_log="variable_log",
        a=1,
        variable_fe="fixed_effects",
        variable_y="var_y",
        variable_x="var_x",
    )  # example using the fixed effects regression
    ...
    assert (
        printed_output
        == "The coefficient of var_x is significantly different from 0 with p-value 0.0"
    )
...
def test_poisson_reg(input_data_poisson_reg):
    ...
    result_fe, params_fe = poisson_reg(
    ...
    result_fe_w, params_fe_w = poisson_reg(
        input_data_poisson_reg,
        "y",
        "x",
        ["index1", "index2"],
    ...
    result_fe_w_irr, params_fe_w_irr, pred, irr_pred = poisson_reg(
        input_data_poisson_reg,
        "y",
        "x",
        ["index1", "index2"],
