...
def _gen_rep_var_fixed_extension_mon(
    ...
    """Generate new variables based on a condition and replace values of the variable
    ...
    for i in range_loop:
        ...
        df[list(list_names_ext)] = original_value_var
    for i in range_loop:
...
def _gen_rep_var_single_cond_biggerthan_mon(
    ...
    """Generate a new variable `var_gen` in a pandas dataframe `df` with an original
    value `original_value`. If the condition variable `cond_var` is greater than the
    condition `cond`, the variable will be replaced with `final_value`.
...
def _gen_rep_var_fixed_extension_mon2(
    ...
    for i in range_loop2:
        ...
        df[list(list_names_ext2)] = original_value_var2
    for i in range_loop2:
        df.loc[df[var_cond_ext2] == i, f"cuad{i}"] = final_value_var2
    return df


def _gen_var_cond_list_similar_mon(df, ori_variables, fixed_variable, name_change="p"):
    """Generates new variables in a dataframe based on existing variables with a similar
    name by multiplying them with a fixed variable.
...
def _gen_rep_var_various_cond_equality_mon(
    ...
    """This function generates a new variable (new_gen_variable) in a dataframe (df)
    with an original value (new_original_value). The values of the new variable are
    replaced with different values (range_new_gen) on a loop for the original list of
    variables (list_ext_variables) depending on different conditions on other variables
    (value_originallist) already existing in the data frame.
...
def _gen_var_double_listed_mon(df, list_gen_var, list_ori_var, fixed_var):
    """This function generates new variables (columns) listed on a list (list_gen_var)
    in a dataframe (df), using a multiplication rule by a fixed variable (fixed_var)
    that already exists in the data frame. The values for the new variables are
    calculated using an existing list of variables (list_ori_var) already present in the
    dataframe (df).
...
def _gen_rep_var_various_cond_equality_listedvalues_mon(
    ...
    """This function generates a new variable (NEW_var) with an original value (ORI_var)
    and then replaces it based on a condition on an existing variable (list_a) in a
    dataframe (df) using elements of a list (list_b) to replace the value.
...
def monthlypanel_1(
    ...
    df = _gen_rep_var_various_cond_equality_mon(
        df,
        new_gen_variable,
        list_ext_variables,
    )
...
def _gen_rep_various_cond_mon2(
    df,
    new_var_v_cond,
    ori_var_v_cond,
    var_con_v_cond,
    ...
    """Generate a new variable in a dataframe based on the values of another existing
    variable and a condition on a variable in the dataframe.
    ...
    df.loc[df[var_con_v_cond] == con_v_cond1, new_var_v_cond] = (
        df[ori_var_v_cond] * multiple1_v_cond
    )
    df.loc[
        (df[var_con_v_cond] == con_v_cond2)
        | (df[var_con_v_cond] == con_v_cond3)
        | (df[var_con_v_cond] == con_v_cond4)
        | (df[var_con_v_cond] == con_v_cond5),
        new_var_v_cond,
    ] = (
        df[ori_var_v_cond] * multiple2_v_cond
    )  # replace
    return df


def _genNA_rep_two_cond_mon2(df, list_for_NA, var_con_NA, fixed_var_NA, NA_value=7):
    """Generates a set of NA variables in a dataframe (df) based on a list of variable
    names (list_for_NA). The value of some of the variables on the list are replaced
    with values of an original variable in the dataframe (fixed_var_NA) based on a list
    of conditions over a variable (var_con_NA). The conditions check whether this
    variable has a value higher or lower than a fixed value (NA_value).
...
def _egen_rep_mon2(
    ...
    """Generates a new variable (new_egen_var) in a dataframe (df) based on the sum of
    an existing variable (var_egen_sup) grouped by a by variable (by_var). Then,
    replaces the values of an existing variable (ege_var_change) in the dataframe with
    the new variable scaled by a given factor (ege_scale_factor) if an existing variable
    (cond_ege_var) meets a certain condition (cond_ege_val).
    ...
    df.loc[df[cond_ege_var] == cond_ege_val, ege_var_change] = (
        df[new_egen_var] / ege_scale_factor
    )
    return df
...
def _egen_rep2_mon2(
    ...
    df.loc[df[cond_ege_var2] == cond_ege_val2, ege_var_change2] = (
        df[new_egen_var2] / ege_scale_factor2
    )
    return df


def _complex_gen_rep_mon2(
    df,
    list_names_complex,
    ...
    value_last_complex=1,
):
    """This function replaces the values of a variable (var_rep_complex) in a dataframe
    (df) by using a condition on another variable already existent in the dataframe
    (var_rep_cond_complex). The condition is based on a list of names
...
def _gen_rep_simple_mon2(
    ...
    """" Generates a new variable in a dataframe and replaces its values based on a
    condition.
...
def _gen_based_variouslists_mon2(
    df,
    list_names_place,
    list_names_month,
    list_names_variouslists,
):
    """This function is firstly extending two lists which in turn will be used to create
    a set of variables (list_names_place) based on the values of two different other
    lists (list_names_month,  list_names_variouslists_m2) multiplied by each other.
    ...
    list_names_place.extend(
        [f"mbelg{i}" for i in ["may", "jun", "jul", "ago", "sep", "oct", "nov", "dec"]],
    )
    list_names_place.extend(
        [
            f"monce{i}"
...
def monthlypanel_2(
    ...
    new_gen_var_sim="w",
    value_sim=0.25,
    new_gen_var_sim2="n_neighborhood",
    value_sim2=0,
    new_generated_var="total_thefts_q",
    existing_var="total_thefts",
    scalar_gen=4,
    list_sort=["observ", "month"],
    var_drop="month",
    drop1=72,
    ...
    df = _gen_based_variouslists_mon2(
        df,
        list_names_place,
        list_names_month,
        list_names_variouslists,
...
def _gen_rep_3cond_mon3(
    ...
    """This function is generating a variable (gen_var_3cond) in a dataframe (df) with
    an initial value initial_val_3cond. Then it is replacing the value of this variable
    if any of three condition on three variables are met (col1_3cond, col2_3cond,
    col3_3cond) The values they need to meet and the value of the new variable in the
    end is the same (global_replace_val_3cond).
    ...
    df.loc[
        (df[col1_3cond] == global_replace_val_3cond)
        | (df[col2_3cond] == global_replace_val_3cond)
        | (df[col3_3cond] == global_replace_val_3cond),
        gen_var_3cond,
    ] = global_replace_val_3cond
    return df


def _gen_multiplevariables_listbased(
    df,
    list_values,
    list_names_3_variables,
    list_names_data3_general,
):
    """This function is list based. Given some list entries (list_value1, list_value2,
    list_value3, list_value4, list_names_3_variables, list_names_data3_general), various
    variables are created in a data frame (df).
    ...
    for i, values in enumerate(list_values):
        for j in range(3):
            df[values[j]] = (
                df[list_names_3_variables[i]] * df[list_names_data3_general[j]]
            )
        for j in range(3, 6):
            df[values[j]] = (1 - df[list_names_3_variables[i]]) * df[
                list_names_data3_general[j]
            ]
    return df
...
def monthlypanel_3(
    ...
    df = _gen_multiplevariables_listbased(
        df,
        list_values,
        list_names_3_variables,
        list_names_data3_general,
...
def _rep_various_mon4(
    df,
    cond_var_simple_v,
    range_replace=range(5, 8),
    val_assigned_various=1,
):
    """Replace multiple variables in a dataframe (df) by looping through a fixed range
    (range_replace) based on a condition that should be met by another variable in the
    dataframe (cond_var_simple_v). If the condition is met, the value assigned to these
    variables is also fixed (val_assigned_various).
...
def _gen_rep_var_various_cond_biggerthan_mon4(
    df,
    list_gen_var_cond,
    cond_var_cond,
    original_value_var_cond=0,
    range_var_cond=range(4, 7),
    final_value_var_cond=1,
):
    """Generate and replace variables in a dataframe based on a loop range and a
    condition on another column.
...
def _gen_specificrule_list(
    ...
    """This function is generating specific new variables (ist_var_gen_spec) in a
    dataframe (df) based on already existing set of variables in the data frame
    (list_var_ext_spec, list_new_var_spec) and this is done over a loop
    range(range_specific_loop).
...
def monthlypanel_4(
    ...
    list_new_var_spec,
    location,
    new_var_new="jewish_inst_one_block_away_1",
    var1_new="jewish_inst_one_block_away",
    var_sub_new="jewish_inst",
    list_various_gen=["cuad2", "month5", "month6", "month7"],
    original_val_various_list=0,
    cond_var_simple="distance_to_jewish_inst",
    var_simple_rep="cuad2",
    cond_val_simple=2,
    val_assigned_simple=1,
    var_drop_cond="month",
    drop_condition_list=[72, 73, 8, 9, 10, 11, 12],
):
    df = _clean_column_names_mon4(df)
    df[new_var_new] = df[var1_new] - df[var_sub_new]
    df[list(list_various_gen)] = original_val_various_list
