...
def WelchTest(Data, code1, code2):
    ...
    """
    Conducts a Welch test to compare the mean values of two specified variables in a dataset.
...
def testings(regression, variable_test, testing_number):
    ...
    """
    Performs a t-test to determine if a given variable in a regression model is significantly different from a specified value.
...
def areg_single(Data, variable_log, a, variable_fe, variable_y, variable_x):
    ...
    reg = smm.OLS(y[m], X[m]).fit(cov_type='cluster', cov_kwds={'groups': fixed_effects}) 
    params = reg.params
    return reg
...
def areg_double(Data, type_condition, variable_loga, variable_logb, a, variable_fe, variable_y, variable_x):
    ...
    if type_condition == "equal":
        ...
        X = smm.add_constant(x)
        reg = smm.OLS(y, X).fit(cov_type='cluster', cov_kwds={'groups': fixed_effects[variable_fe]})
        params = reg.params
        return reg
...
def reg_robust(Data, variable_y, variable_x):
    ...
    """
    Performs a robust regression using Huber's T norm, which gives less weight to outliers and more weight to inliers when 
    calculating the scale of the data.
...
def areg_clus_abs(Data, drop_subset, y_variable, x_variable, dummy_variable):
    ...
    """
    Perform a simple regression with clusters on a dataset using a specified dummy variable for clustering.
    ...
    reg = smm.OLS(y, X).fit(cov_type='cluster', cov_kwds={'groups': df[dummy_variable]}, use_t=True) 
    params = reg.params
    return reg
...
def poisson_reg(Data, y_variable, x_variable, index_variables, type_of_possion, weight, x_irra):
    ...
    """
    Performs a fixed effects poisson regression for different types of conditions reflected in (type_of_poisson);
    whether it is a simple fixed effects poisson regression, or is it weighted by a variable, in which case we input
    (weight) or is if we need to calculate interrater reliability (IRR) coefficients for a given set of data we add
    (x_irra).
    ...
    if type_of_possion == "fixed effects":
        ...
        w = Data[weight] 
        data['iweight'] = w 
        model = PanelOLS(Y, X, entity_effects=True, time_effects=True, drop_absorbed=True, weights=data['w']) 
        reg = model.fit(cov_type='clustered', cluster_entity=True)
        params = reg.params
        return reg, params
